     ...
     (if (is-valid myConditionsStr groups)
       (if (= (count unknowns) (count unknownLocs))
         ...
         (if (contains? cache state)
           ...
           (list (get cache state) cache)

           (let [[leftAcc leftCache] (count-solutions2 row (conj unknowns \#) cache)
                 [rightAcc rightCache] (count-solutions2 row (conj unknowns \.) leftCache)]
             ;(println "branch" " | " state " | " myConditionsStr " |la " leftAcc " |ra " rightAcc " |lc " leftCache " |rc " rightCache " |c " cache)
             (list (+ leftAcc rightAcc)
                   (assoc rightCache state (+ leftAcc rightAcc))))))
                  ;;  (if (seq (first state))
                  ;;    (assoc rightCache state [leftAcc rightAcc])
                  ;;    rightCache)))))
       ;(do (println "stop" " | " state " | " myConditionsStr " | " acc " | " cache)
         (list 0 cache)) ; abandon this path
       )))

(defn part2 []
  (->> (parse-input2 fname)
       (pmap count-solutions2)
       (pmap first)
       sum))
